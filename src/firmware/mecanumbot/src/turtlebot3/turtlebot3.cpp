...
static const TB3ModelInfo mecanum_info = {
  ...
  0.033,
  0.287,
  0.287, // wheel_distance_y is not measured yet
  0.1435,
  0.220,
  false,
};
...
void TurtleBot3Core::begin(const char* model_name)
{
  ...
  if(strcmp(model_name, "Burger") == 0 || strcmp(model_name, "burger") == 0){
    ...
    model_motor_rpm = 77;
  }else if(strcmp(model_name, "Mecanum") == 0 || strcmp(model_name, "mecanum") == 0){
  ...
  dxl_slave.addControlItem(ADDR_PRESENT_POSITION_L, control_items.present_position[MortorLocation::LEFT_FRONT]);
  dxl_slave.addControlItem(ADDR_PRESENT_POSITION_R, control_items.present_position[MortorLocation::RIGHT_FRONT]);
  dxl_slave.addControlItem(ADDR_PRESENT_VELOCITY_L, control_items.present_velocity[MortorLocation::LEFT_FRONT]);
  dxl_slave.addControlItem(ADDR_PRESENT_VELOCITY_R, control_items.present_velocity[MortorLocation::RIGHT_FRONT]);
  dxl_slave.addControlItem(ADDR_PRESENT_CURRENT_L, control_items.present_current[MortorLocation::LEFT_FRONT]);
  dxl_slave.addControlItem(ADDR_PRESENT_CURRENT_R, control_items.present_current[MortorLocation::RIGHT_FRONT]);
  ...
  dxl_slave.addControlItem(ADDR_PROFILE_ACC_L, control_items.profile_acceleration[MortorLocation::LEFT_FRONT]);
  dxl_slave.addControlItem(ADDR_PROFILE_ACC_R, control_items.profile_acceleration[MortorLocation::RIGHT_FRONT]);
...
void update_motor_status(uint32_t interval_ms)
{
  ...
  int16_t current_lf, current_rf, current_lr, current_rr;
  ...
  if(millis() - pre_time >= interval_ms){
    ...
    pre_time_dxl = millis();
    if(get_connection_state_with_motors() == true){
      motor_driver.read_present_position(control_items.present_position[MortorLocation::LEFT_FRONT], control_items.present_position[MortorLocation::RIGHT_FRONT], control_items.present_position[MortorLocation::LEFT_REAR], control_items.present_position[MortorLocation::RIGHT_REAR]);
      motor_driver.read_present_velocity(control_items.present_velocity[MortorLocation::LEFT_FRONT], control_items.present_velocity[MortorLocation::RIGHT_FRONT], control_items.present_velocity[MortorLocation::LEFT_REAR], control_items.present_velocity[MortorLocation::RIGHT_REAR]);
      if(motor_driver.read_present_current(current_lf, current_rf, current_lr, current_rr) == true){
        control_items.present_current[MortorLocation::LEFT_FRONT] = current_lf;
        control_items.present_current[MortorLocation::RIGHT_FRONT] = current_rf;
        control_items.present_current[MortorLocation::LEFT_REAR] = current_lr;
        control_items.present_current[MortorLocation::RIGHT_REAR] = current_rr;
      }
...
static void dxl_slave_write_callback_func(uint16_t item_addr, uint8_t &dxl_err_code, void* arg)
{
  ...
  switch(item_addr)
  {
    ...
    case ADDR_PROFILE_ACC_R:
      if(get_connection_state_with_motors() == true)
        motor_driver.write_profile_acceleration(control_items.profile_acceleration[MortorLocation::LEFT_FRONT], control_items.profile_acceleration[MortorLocation::RIGHT_FRONT], control_items.profile_acceleration[MortorLocation::LEFT_REAR], control_items.profile_acceleration[MortorLocation::RIGHT_REAR]);
      break;
...
void test_motors_with_buttons(uint8_t buttons)
{
  ...
  if(get_connection_state_with_motors() == true){
    motor_driver.read_present_position(current_tick[MortorLocation::LEFT_FRONT], current_tick[MortorLocation::RIGHT_FRONT], current_tick[MortorLocation::LEFT_REAR], current_tick[MortorLocation::RIGHT_REAR]);
  }
